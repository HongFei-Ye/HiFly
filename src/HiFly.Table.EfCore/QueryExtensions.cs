...
namespace HiFly.Table.Extensions;
...
public static class QueryExtensions
{
    ...
    /// <returns></returns>
    /// <exception cref="NotImplementedException"></exception>
    public static Expression? RecursiveBuildAutoExpression_Old<TItem>(PropertyFilterParameters filter, ParameterExpression parameter)
    {
        Expression? combinedExpression = null;
    ...
    public static Expression? RecursiveBuildAutoExpression<TItem>(PropertyFilterParameters filter, ParameterExpression parameter)
    {
        ...
        if (filter.Filters != null && filter.Filters.Count != 0)
        {
            foreach (var subFilter in filter.Filters)
            {
                // 使用递归调用，不能直接调用 BuildAutoExpression
                var subExpression = RecursiveBuildAutoExpression<TItem>(subFilter, parameter);
                if (subExpression != null)
                {
                    // 【修复】使用父过滤器的 FilterLogic 来组合子表达式
                    combinedExpression = combinedExpression switch
                    {
                        null => subExpression,  // 如果没有组合的表达式，则直接使用子表达式
                        _ => filter.FilterLogic switch  // 使用父过滤器的 FilterLogic 来组合
                        {
                            FilterLogic.And => Expression.AndAlso(combinedExpression, subExpression),
                            FilterLogic.Or => Expression.OrElse(combinedExpression, subExpression),
                            _ => throw new NotImplementedException($"未实现 FilterLogic 的 '{filter.FilterLogic}' 这个参数")
    ...
    private static Expression? ValueTypeBuildExpression(PropertyFilterParameters filter, ParameterExpression parameter, bool isRecursive = false)
    {
        ...
        if (filter.ValueTypeField != null)
        {
            // 支持嵌套属性路径（如 "Material.Name"）
            var property = BuildNestedPropertyExpression(parameter, filter.ValueTypeField);
            var conditionExpression = GetConditionExpression(property, filter.MatchValue, filter.FilterAction);
    ...
    /// <summary>
    /// 构建嵌套属性表达式（支持 "Material.Name" 这样的路径）
    /// </summary>
    /// <param name="parameter">参数表达式</param>
    /// <param name="propertyPath">属性路径，支持用 "." 分隔的嵌套路径</param>
    /// <returns>属性访问表达式</returns>
    private static MemberExpression BuildNestedPropertyExpression(Expression parameter, string propertyPath)
    {
        ...
        // Expression.Property 总是返回 MemberExpression
    ...
    {
        Expression constantExpression = Expression.Constant(matchValue, memberExpression.Type);
